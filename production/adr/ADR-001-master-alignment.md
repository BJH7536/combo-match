...
- **O-2** (격하 — 필수 아님): `game/play.html` 등 누락 파일 4종은 **진행에 불요** — 규칙은
  마스터 §4 + `runOneSim`으로 완결, 아이템 UX·전달 채널도 §4.3·§8에 명시. 받으면 참고용.
  우리 게임이 동일 해시 포맷(`#level=<base64>`)을 수용하면 디자이너 툴과 직접 연동된다.
