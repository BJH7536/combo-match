...
- **O-2** (**해소 — 2026-07-26 수신·대조 완료**): 요청한 4종(`game/play.html`,
  `levels/sample-level-1234.json`, `difficulty-elements.md`, `level-mechanics-brainstorm.md`)
  전부 수신. 대조 결과:
  - 이미 보유한 마스터 GDD·디자이너 툴은 **바이트 단위 동일** — 기존 정렬·엔진 동치성 유효.
  - `play.html`은 별개 구현이 아니라 같은 규칙의 UI 판. 엔진 대비 **미구현분은 세션/경제
    계층뿐**(골드 `combo-match:gold`, 힌트 120·집게 350·와일드 500 구매, localStorage
    playtest 채널 + storage 이벤트 라이브 갱신, 파일 열기, `?level=` 쿼리). 규칙 로직 차이 없음.
  - `sample-level-1234.json`은 **level@1**(F계층 필드·rules 전무). 우리 로더가 이를 거부해
    레퍼런스(`liteFromLevelJson`의 `||0`/`||[]`/`!!` 정규화)와 불일치했음 → **로더를
    "부재 허용·정규화, 값 있으면 엄격 검증"으로 수정**하고 `LevelCardData`(입력, F계층 optional)와
    `RuntimeCard`(런타임, 전 필드 필수)를 분리. 회귀 테스트 `tests/level-compat.test.ts`.
  - 브레인스토밍 문서의 추가 장치 후보(오염 전파·다중 커버·골드 카드·쌍둥이)는 마스터
    7종 확정 범위 밖 — 채택하지 않음.
